...
def intron_mafft_conspos(
    ...
    # TODO: For introns change functions here to call mafft_ginsi_align, mafft_linsi_align and mafft_einsi_align.
    ginsi_aln_path = os.path.join(
        os.path.dirname(aln_path), os.path.basename(nucl_path) + '.ginsi')
    linsi_aln_path = os.path.join(
        os.path.dirname(aln_path), os.path.basename(nucl_path) + '.linsi')
    einsi_aln_path = os.path.join(
        os.path.dirname(aln_path), os.path.basename(nucl_path) + '.einsi')

    mafft_ginsi_align(
        infile = nucl_path, output_aln = ginsi_aln_path  + '.fna.aln',
        iterations=aln_iterations)
    mafft_linsi_align(
        infile = nucl_path, output_aln = linsi_aln_path + '.fna.aln', iterations=aln_iterations)
    mafft_einsi_align(
        infile = nucl_path, output_aln = einsi_aln_path + '.fna.aln', iterations=aln_iterations)
